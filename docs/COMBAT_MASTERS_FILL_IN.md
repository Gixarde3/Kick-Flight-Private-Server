...
| `masters_skill_heal.json` | `skillId`, `skillHealType`, `coefficient`. A skill may have several rows (`SupportSkillAction.Heal` walks `HealInfos`). **1 Hp**: `floor(MaxHP × coef)` (0.5 = 50 %), to self or every ally depending on the APK `ActionMaster._targetCount`; **2 BoostGauge**: `coef × 120` boost points; **5 Condition**: cleanses the healable status effects (Bullheader/Pegasia = a type-1 row + a type-5 row). Warp discs heal `MaxHP × coef` of their first row on arrival (`WarpSkillAction.EndFinish`). **3 HpDrain / 4 SpDrain** are read by `PlayerCharacter.ApplyHealOnDamage` from the first row of the skill that just dealt damage: 3 heals `coef × damage dealt` ("recovers X % of dealt damage"), 4 steals `min(coef × 100, SP)` special-skill gauge from the victim (Dragarmr 0.2 = +20 %, Hell Saurer 0.02 × 20 hits). |
| `masters_skill_condition.json` | `skillId`, `conditionType`, `duration` (s), `interval` (s, 0 = one shot; for Dot/Poison/Regeneration ticks), `effectValue`, `triggerType`. **3 Execute** = on the caster (or every ally for `_targetCount` 4 skills) at cast — only `SupportSkillAction`, `WarpSkillAction`, `AutoMove`/`VerticalLoop` apply it, attack discs never do; **1 ReceiveDamage** = on whoever the hit lands on (the rows travel inside the `DamageInfo`, filtered by trigger on the receiving side); 4/5/6 = trap enter. Values: 1 AttackRate / 2 SpeedRate / 4 DamageRate (incoming-damage multiplier, `PlayerCharacter.AcceptDamageInfo`; 3 DefenseRate is only shown on the member icon) / 28 AttackSpeedRate / 31 BoostHealSpeedRate are multipliers (0.3 = −70 % damage taken); 5 Poison ticks `target MaxHP × value` every `interval`; 17 Dot ticks `setter attack × value`; 11 Regeneration heals `MaxHP × value` per tick; 6 Paralysis / 8 Stun / 9 Silent ignore the value; 32 SpecialSkillGaugeRate removes `(1 − value) × MaxSP` once (0.8 = −20 %). |
| `masters_skill_blow_off.json` | `skillId`, `distance`, `speed`, `rigorTime` (stun after landing), `directionType` (1 Up = launch, 2 Down = slam, 4 AttackDirection = pushed along victim − attacker; `FrontAttackSkillAction` falls back to **Down** without a row, so "knocks upwards" short-range discs need an Up row). On a **MoveAttack** disc the row also turns the dash into a "pursuit" that carries the victim along (`MoveAttackSkillAction.HitCallback`, entrained blow-off) instead of piercing, and `DiscSkillParameter..ctor` only flags MoveAttack/FrontAttack hits as **shield break** when the skill has neither a blow-off nor a heal row — the card split "dash pierce & shield break" vs "dash pursuit". |
| `masters_skill_pull_in.json` | `skillId`, `distance`, `speed` → `DamageInfo.PullInInfo` → `PlayerCharacter.ApplyPullIn` → `PlayerStatePullIn` towards the attacker (Yuyan's and Anna's kicker skills). Ignored when the same skill has a blow-off row. |
| `masters_skill_trap.json` | `skillId`, `trapType`, `duration`, `radius`, `effectValue` (e.g. 0.5 = half speed for Slow), `interval`, `executeSeId`, `effectPath`, `screenEffectPath` (fullscreen effect for whoever steps in). For disc traps `DiscSkillParameter..ctor` rebuilds the `TrapInfo` from the APK sensor-collider clip (duration = clip `_time`, radius/explosion collider/effect from the child collider clip), so the served duration/radius are only fallbacks. **Jay's bomb (40001, trapType 10)** has no timeline group in any bundle, so the served row *is* the TrapInfo: `duration` = seconds until it explodes (`BatBombTrapAction.OnManagedUpdate`, timer only — IsTimeExecute is forced), and the production cave "BatAbilityParameter..ctor tail" (`patch-il2cpp-endpoints.py`) maps `radius` → blast end radius (2 → radius over `interval` s), `effectPath` → explosion SPFX, collider = `skill_40001` collision 198; blast damage = attack × `Skill.coefficient` (10). **Sid's statue (20014, trapType 4 Decoy)**: `LaserSkillAction.CreateTrap` sends the trap with the kicker skill's `TrapInfo` = this row, and `Trap.Initialize` always builds a `StatueTrapAction` for it (`TrapParameter.IsForceTarget` = owner weapon Laser + AttackType KickerSkill; the statue is Sid's weapon model scaled by the asset `KickerSkillMaster.Laser.StatueScale`, turrets/bombs/guardian AI retarget onto it). Without the row `TrapActionBase.Initialize` NREs on `Trap.Param.Info` and nothing appears. `duration` = statue lifetime; the trigger-3 `AttackSpeedRate 2.0` condition row of the same skill is his "+100 % attack speed while the statue stands" passive (`LaserSkillAction.OnBeginAction` applies the trigger-3 rows to Sid; the 2.11.0 `LaserAbilityParameter` itself only shortens the cooldown on shield hits). |
...
## Special-skill gauge — `masters_kicker_parameter.json`

`PlayerCharacter.CalcChargeSP`: `addMoveSpecialSkillPoint` = SP per unit of distance flown (case 2, every frame while
moving), `addWeaponAttackSpecialSkillPoint` = SP per basic-attack hit (case 3); both × the ability `SPRecoveryRate`;
`GetMaxSP` is 100. The served values are `100 / distance-to-full` from games.app-liv.jp/archives/431798 (Jay 550,
Buzzy 630, Owlbert/Pitophy 650, Tsubame 740, Grenhawk 760, Ruriha/Coco 800, Kite 845, Diatrius 900, Hitagi 960,
Yuyan 1180, Sid 1350, Anna 1500 units).
