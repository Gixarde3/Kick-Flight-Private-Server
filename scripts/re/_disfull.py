"""_disfull.py <rva_hex> [max_insns] [--so path]  (a64dis.py without the stop at the first `ret`)
...
TypeInfo or Method* they load. Does NOT stop at `ret`, so whole functions (and their tail blocks) can be listed.
